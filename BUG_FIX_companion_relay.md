...
### W2 — DRIFT: multi-adapter (dual-NIC) improvement documented but NOT in committed config
`system_companion.md:527, 817` (release v1.0.9, commit `ea17fe4`) describe moving video to `service_type = udp_proxy` with **two NICs** (`WFB_NICS="wlx782288d993c0 wlx782288d98f91"`) for TX across both adapters. But the committed config still ships single-adapter:
- `wifibroadcast.cfg:58` → video stream `service_type = 'udp_direct_tx'` (single NIC only)
- `etc/default/wifibroadcast` → `WFB_NICS="wlx782288d993c0"` (one NIC)

**Fix:** reconcile — if dual-NIC is the intended production setup, commit `udp_proxy` + both NICs; otherwise correct the docs. Right now the "improvement" isn't actually deployed.
...
## Suggested order
1. ~~**W1** channel mismatch~~ — ✅ RESOLVED 2026-07-10: not a live bug, just stale GitHub mirror (live link is on 161 both sides). Remaining: run `relay_git_sync.sh` on companion when it has internet to fast-forward the mirror. Downgraded from CRITICAL.
2. **C6** broken companion mavlink-router unit — telemetry may be running on defaults/crash-looping. **← now the top real bug to verify.**
3. **W2** reconcile dual-NIC config vs docs.
4. **C1/C2** stop leaking secrets (PIN + sync filter), then rotate.
5. **W3/W4** tuning pass (MCS, short_gi, agg timeout) — test on hardware.
6. **C7 + W5** persist WFB stats → GCS chip, then adaptive bitrate (biggest ongoing win).
